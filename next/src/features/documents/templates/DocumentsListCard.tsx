...
import { UseGetResult, UseMutationResult } from '@/cdn/types/Query';
...
const DocumentsListCard = <T,>(props: DocumentsListCard<T>) => {
  ...
  let documentCreateMutation: UseMutationResult<FormData> | undefined =
    undefined;
  let documentDeleteMutation: UseMutationResult<any> | undefined = undefined;

  if (props.mediaType === Medias.ANIMAL_DOCUMENT) {
    documentCreateMutation = useAnimalDocumentCreateMutation(
      props.documentsQuery.refetch,
      token?.token
    );
    documentDeleteMutation = useAnimalDocumentDeleteMutation(
      props.documentsQuery.refetch,
      token?.token
    );
  }
  ...
  const onDocumentCreateSubmit = (formData: FormData) => {
    if (documentCreateMutation) {
      documentCreateMutation.mutate(formData);
    }
  };

  const onDocumentDeleteSubmit = (entityId: number) => {
    if (documentDeleteMutation) {
  ...
  const Toolbar = (
    <>
      {props.relatedEntityId && documentCreateMutation && (
        <Button
          label="Ajouter"
    ...
    <Card title="Documents" toolbar={Toolbar}>
      {props.relatedEntityId && documentCreateMutation && (
        <DocumentUploadDialog
          ...
          createQuery={documentCreateMutation}
        />
      )}
      <List>
